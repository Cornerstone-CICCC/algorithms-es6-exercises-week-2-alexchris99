...
but they have no clue how to much blow them up. Each attraction needs to be pumped to a 
precise volume to achieve maximum festival fun!

The attractions are each made up of a combination of several different shapes: cones, 
spheres and prisms. For example, the giant inflatable duck is made up of two spheres 
(the body and head) and a cone (the beak) 🦆.
...
const totalVolume = function (solids) {
  let total = 0
  total += sphereVolume(solids.largeSphere["radius"])
  total += sphereVolume(solids.smallSphere["radius"])
  total +=  coneVolume(solids.cone["radius"], solids.cone["height"])
  return total
};
...
console.log(272000 < totalVolume(solids) && totalVolume(solids) < 275000); //true

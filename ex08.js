...
if there are multiple sets of values each set should be separated by a comma. If there is only one set of values then you should omit the 
comma.
*/

const repeatNumbers = function (data) {
  let newstring = data.map(item =>{
    let string = " "
    for(let i = 0; i < item[1]; i++){
      string += item[0].toString()
    }
    return string
  })
  return newstring
